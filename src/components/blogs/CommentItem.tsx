...
interface CommentItemProps {
  ...
  replyToComment: (id: string, author: Author, content: string) => Promise<Comment>
  reportComment: (id: string) => Promise<void>
  session: any
}

export default function CommentItem({ comment, depth, likeComment, replyToComment, reportComment, session }: CommentItemProps) {
    ...
    if (replyContent.trim()) {
      try {
        await replyToComment(
          comment._id,
          { name: session.user.name || 'Anonymous', email: session.user.email || '' },
          replyContent
        )
        setReplyContent("")
        setIsReplying(false)
